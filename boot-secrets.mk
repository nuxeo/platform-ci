...
.tmp/boot-secrets.mk:
	@( yq r .tmp/boot-secrets.yaml --printMode pv 'secrets.adminUser.*' | awk -f .tmp/boot-secrets-admin-user.awk; yq r .tmp/boot-secrets.yaml --printMode pv 'secrets.pipelineUser.*' | awk -f .tmp/boot-secrets-pipeline-user.awk; yq r .tmp/boot-secrets.yaml --printMode pv 'secrets.hmacToken' | awk -f .tmp/boot-secrets-hmac-token.awk ) > $(@)
